...
namespace Packages.SimpleDragAndDrop.Scripts {
	...
	{
		[Tooltip("Defines section of the cell (armor or gun)")]
		public DragAndDrop.Section section;
		[Tooltip("Defines slot of the cell (head, gun, leg, gun, etc.)")]
		public DragAndDrop.Slot slot;
		...
		public void OnDrag(PointerEventData data)
		{
			if (icon != null)
			{
				icon.transform.position = new Vector3(Mouse.current.position.x.ReadValue(), Mouse.current.position.y.ReadValue(), 0);                          // Item's icon follows to cursor in screen pixels
			}
		}
